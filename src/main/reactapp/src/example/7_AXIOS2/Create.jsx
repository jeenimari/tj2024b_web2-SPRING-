...
export default function Create(props){
    //[1]form에서 입력받은 데이터들을 저장하는 상태변수,초기값 설정
    const [productForm, setProductForm] = useState({ name: '', price: '', comment: '' })
    
    //[2]입력 이벤트 만들기 e.target.속성명:현재 이벤트가 발생한 DOM의 속성값 반환
    const onValueChange = (e) => { //e:event약어 : onchange실행 결과 정보를 매개변수로 받는다.
        //스프레드연산자,{...기존객체,새로운속성명:새로운값}
        setProductForm({...productForm, [e.target.name]: e.target.value});
    }
    
    //[3]제품등록 버튼을 클릭했을때. , axios 이용하여 서버와 통신하고 , 
    // navigate 이용하여 가상URL 페이지 전환한다.
